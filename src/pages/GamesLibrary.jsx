  ...
  if (!inLauncher) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <h1 className="font-display text-2xl font-bold text-white">{t('games.library', 'Biblioteca')}</h1>
        <p className="mx-auto mt-2 max-w-md text-sm text-zinc-400">
          {t('games.libraryDesc', 'A biblioteca de jogos instalados aparece dentro do launcher Filmerama Games.')}
        </p>
        <Link to="/download" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-accent-green px-5 py-2.5
          ...
          <Download className="h-4 w-4" /> {t('games.downloadTheLauncher', 'Baixar o launcher')}
        </Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="flex items-center gap-2 font-display text-2xl font-bold tracking-tight text-white">
          <LibraryBig className="h-6 w-6 text-accent-green-bright" /> {t('games.library', 'Biblioteca')}
        </h1>
        <button onClick={() => openGamesFolder()}
          ...
          <FolderOpen className="h-4 w-4" /> {t('games.openFolderShort', 'Abrir pasta')}
        </button>
      </div>
      ...
      {items === null ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-card-lg border border-dashed
          ...
          <h2 className="font-display text-lg font-semibold text-white">{t('games.libEmptyTitle', 'Nenhum jogo instalado ainda')}</h2>
          <p className="mt-2 max-w-md text-sm text-zinc-400">{t('games.libEmptyMessage', 'Instale um jogo pelo catálogo e ele aparece aqui.')}</p>
          <Link to="/games/catalogo" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-accent-green px-4 py-2
                                                text-sm font-bold text-cinema-bg shadow-glow-green hover:bg-accent-green-bright">
            {t('games.viewCatalog', 'Ver catálogo')}
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it) => {
            const title = it.meta?.title || `${t('games.gameLabel', 'Jogo')} ${it.gameId}`;
            const cover = (() => {
              ...
              <div key={it.gameId} className="overflow-hidden rounded-card bg-cinema-surface ring-1 ring-white/5 shadow-poster">
                ...
                <div className="flex items-center justify-between gap-3 p-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-zinc-500">{it.version ? `v${it.version}` : t('games.installed', 'instalado')}</p>
                  </div>
                  <button onClick={() => gameAction('play', it.gameId)} disabled={running}
                    className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-accent-green px-3 py-1.5 text-xs
                               ...
                               disabled:opacity-60">
                    <Play className="h-3.5 w-3.5 fill-current" /> {running ? t('games.running', 'Rodando…') : t('games.play', 'Jogar')}
                  </button>
                </div>
              </div>
            );
