...
import { FaUnity, FaRobot, FaMicrophone, FaSolarPanel, FaServer, FaHandshake, FaGraduationCap, FaWhatsapp, FaInstagram, FaYoutube, FaFacebook } from 'react-icons/fa';
...
export default function Studio() {
  ...
  const specialties = [
    { icon: <MdAnimation className="text-4xl text-purple-600" />, title: t('studioPage.animation'), desc: t('studioPage.animationDesc') },
    { icon: <MdGames className="text-4xl text-green-600" />, title: t('studioPage.games'), desc: t('studioPage.gamesDesc') },
    { icon: <MdRecordVoiceOver className="text-4xl text-purple-600" />, title: t('studioPage.voiceOver'), desc: t('studioPage.voiceOverDesc') }
  ];
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex gap-4 mb-8 border-b border-gray-200 dark:border-gray-800">
            <button
              ...
              className={`pb-3 px-4 font-semibold transition-colors ${activeTab === 'about' ? 'border-b-2 border-purple-600 text-purple-600' : 'text-gray-600 dark:text-gray-400'}`}
            >
              {t('studioPage.about')}
            </button>
            <button
          ...
          {activeTab === 'about' && (
            <div className="space-y-12">
              {/* Especialidades */}
              <section>
                <h2 className="text-3xl font-bold mb-8 text-center">{t('studioPage.specialties')}</h2>
                <div className="grid md:grid-cols-3 gap-8">
                  {specialties.map((item, index) => (
                    <div key={index} className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm p-6 rounded-xl shadow-lg text-center">
              ...
              {/* Estrutura */}
              <section>
                <h2 className="text-3xl font-bold mb-8 text-center">{t('studioPage.structure')}</h2>
                <div className="grid md:grid-cols-3 gap-6">
                  <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm p-6 rounded-xl shadow-lg">
                    <FaRobot className="text-3xl text-purple-600 mb-3" />
                    <h3 className="text-xl font-semibold mb-2">{t('studioPage.agile')}</h3>
                    <p className="text-gray-600 dark:text-gray-400">{t('studioPage.agileDesc')}</p>
                  </div>
                  <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm p-6 rounded-xl shadow-lg">
                    <FaSolarPanel className="text-3xl text-green-600 mb-3" />
                    <h3 className="text-xl font-semibold mb-2">{t('studioPage.sustainability')}</h3>
                    <p className="text-gray-600 dark:text-gray-400">{t('studioPage.sustainabilityDesc')}</p>
                  </div>
                  <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm p-6 rounded-xl shadow-lg">
                    <FaServer className="text-3xl text-purple-600 mb-3" />
                    <h3 className="text-xl font-semibold mb-2">{t('studioPage.infrastructure')}</h3>
              ...
              {/* Galeria de Fotos */}
              <section>
                <h2 className="text-3xl font-bold mb-8 text-center">{t('studioPage.gallery')}</h2>
                <div className="grid md:grid-cols-3 gap-6">
                  {studioImages.map((img, index) => (
                    <div key={index} className="bg-gray-200/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl overflow-hidden">
                      <div className="aspect-video bg-gray-300/90 dark:bg-gray-700/90 flex items-center justify-center">
                        {img.url ? (
                          <img 
                            src={img.url} 
                            alt={img.title}
          ...
          {activeTab === 'clients' && (
            <div className="space-y-12">
              <section className="bg-green-50/90 dark:bg-green-900/20 backdrop-blur-sm rounded-xl p-8">
                ...
                <FaHandshake className="text-6xl text-purple-600 mx-auto" />
              </section>

              <section>
                <h2 className="text-3xl font-bold mb-6 text-center">{t('studioPage.schoolProject')}</h2>
                <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-xl p-8">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <h3 className="text-2xl font-semibold">{t('studioPage.labteca')}</h3>
                  </div>
